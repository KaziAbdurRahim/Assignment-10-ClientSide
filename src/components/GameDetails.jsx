...
const GameDetails = ({ poster, title, genres, duration, year, rating, summary, _id }) => {
              ...
              try {
                const response = await fetch(`http://localhost:5000/games/${_id}`, {
                  ...
                  if (response.ok) {
                      ...
                      navigate('/allgames'); // Navigate immediately after showing the success message
                  } else {
                      const errorData = await response.json();
                      toast.error(`Failed to delete the game: ${errorData.message}`);
                  }
              } catch (error) {
                  setIsLoading(false);
      ...
      try {
          ...
          if (response.ok) {
              Swal.fire({
                  ...
                  text: "Your Game has been added to favorites.",
                  icon: "success",
                  timer: 1500, // Automatically closes the alert after 1.5 seconds
                  showConfirmButton: false, // Hides the OK button
              });
  ...
  const  handleUpdate = async () => {
      setIsLoading(true);
      await navigate(`/games/update/${_id}`);
      setIsLoading(false);
  };
